"""Low-volume intermediary (Luna/Claude) smoke through production ASGI composition.

This module tests LUNA_GPT_56 (OPENAI_RESPONSES) and CLAUDE_SONNET_5 (ANTHROPIC_MESSAGES).
...
def _profile_for_runtime(profile: ExternalProfile) -> ModelProfile:
    ...
    if runtime_protocol is None:
        raise ExternalGateError(
            f"unexpected intermediary protocol {profile.protocol} for {profile.alias}"
        )
...
def _profile_by_alias(config: ExternalConfig, alias: str) -> ExternalProfile:
    ...
    if len(matches) != 1:
        raise ExternalGateError(f"Intermediary matrix must contain exactly one {alias}")
    return matches[0]
...
def _create_adapter(profile: ModelProfile, http_client: Any) -> Any:
    ...
    elif profile.protocol == ProviderProtocol.ANTHROPIC_MESSAGES:
...
def _record_entry(
    *,
    ...
    return LedgerEntry(
        scenario_id=f"wo-002-st-002-{profile.alias.lower()}",
        alias=profile.alias,
...
@pytest.mark.parametrize("alias", INTERMEDIARY_ALIASES)
def test_intermediary_smoke_persists_redacted_evidence(
    alias: str,
    external_config: ExternalConfig,
    temporary_resources: Any,
    intermediary_budget: BudgetCounter,
) -> None:
    ...
    reservation = intermediary_budget.reserve(alias, MAX_OUTPUT_TOKENS)
