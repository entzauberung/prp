"""Low-volume VanyoSpace smoke through production ASGI composition.

This module tests OPENAI_RESPONSES protocol only (VanyoSpace limitation).
...
def vanyospace_budget() -> BudgetCounter:
...
def _profile_for_runtime(profile: ExternalProfile) -> ModelProfile:
    ...
    if profile.protocol != "OPENAI_RESPONSES":
        raise ExternalGateError(
            f"VanyoSpace only supports OPENAI_RESPONSES protocol, got {profile.protocol}"
        )
...
def _profile_by_alias(config: ExternalConfig, alias: str) -> ExternalProfile:
    ...
    if len(matches) != 1:
        raise ExternalGateError(f"VanyoSpace matrix must contain exactly one {alias}")
    return matches[0]


def _create_adapter(profile: ModelProfile, http_client: Any) -> Any:
    """Create the OpenAI RESPONSES adapter."""
    if profile.protocol != ProviderProtocol.OPENAI_RESPONSES:
        raise ExternalGateError(f"VanyoSpace only supports OPENAI_RESPONSES")
...
def _record_entry(
    *,
    ...
    return LedgerEntry(
        scenario_id=f"wo-001-st-004-{profile.alias.lower()}",
        alias=profile.alias,
...
@pytest.mark.parametrize("alias", VANYOSPACE_ALIASES)
def test_vanyospace_multi_protocol_smoke_persists_redacted_evidence(
    alias: str,
    external_config: ExternalConfig,
    temporary_resources: Any,
    vanyospace_budget: BudgetCounter,
) -> None:
    ...
    reservation = vanyospace_budget.reserve(alias, MAX_OUTPUT_TOKENS)
